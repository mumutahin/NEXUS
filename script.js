...
const phones = [
  {
    ...
    features: ["Titanium design", "Action button", "ProRes Video", "Face ID"],
...
function openModal(id) {
  ...
  document.getElementById("modal-price").textContent = "$" + currentPhone.price;
  const oldEl = document.getElementById("modal-price-old");
  oldEl.textContent = currentPhone.oldPrice ? "$" + currentPhone.oldPrice : "";
  // Specs
  ...
  const colorsHtml = currentPhone.colors
    .map(
      (c, i) => `
    < div class="color-option ${i === 0 ? "selected" : ""}" style = "background:${c}" onclick = "selectColor(this)" title = "${c}" ></ > `,
    )
    .join("");
  ...
  const storageHtml = currentPhone.storage
    .map(
      (s, i) => `
        < button class="btn btn-outline btn-sm ${i === 0 ? "" : "opacity"}" onclick = "selectStorage(this)" style = "${i === 0 ? "border - color: var(--accent); color: var(--accent)" : ""}" > ${s}</ > `,
    )
    .join("");
  document.getElementById("modal-storage").innerHTML = storageHtml;
  // Wishlist btn
  const isWished = wishlist.find((w) => w.id === currentPhone.id);
  document.getElementById("modal-wish-btn").textContent = isWished ? "❤️" : "♡";
...
function renderComparison() {
  ...
  selected.forEach((p) => {
    html += `< th > <div class="phone-col-header"><div class="phone-col-icon">${p.icon}</div><div><div class="phone-col-name">${p.name}</div><div class="phone-col-brand">${p.brand}</div></div></div></ > `;
  });
  html += "</tr></thead><tbody>";
  compareRows.forEach((row) => {
    ...
    selected.forEach((p, i) => {
      ...
      html += `< td class="${isWinner ? "compare - winner" : ""}" > ${p[row.key]}</ > `;
    });
    html += "</>";
  });
  html += "</tbody>";
...
function filterAcc(cat, btn) {
  ...
  const data =
    cat === "all" ? accessories : accessories.filter((a) => a.cat === cat);
